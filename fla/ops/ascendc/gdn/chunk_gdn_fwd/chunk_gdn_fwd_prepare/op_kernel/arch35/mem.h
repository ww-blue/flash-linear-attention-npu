 ...
 *
 * On-chip buffer views (UB / L1 / L0A / L0B / L0C).
 */

#ifndef CHUNK_GDN_FWD_PREPARE_MEM_H
#define CHUNK_GDN_FWD_PREPARE_MEM_H
...
template <BufferType bufferType, typename Dtype>
__aicore__ inline AscendC::LocalTensor<Dtype> PrepareView(uint32_t offset)
{
    OnChipBuffer buf;
    return buf.template GetBuffer<bufferType, Dtype>(offset);
}
