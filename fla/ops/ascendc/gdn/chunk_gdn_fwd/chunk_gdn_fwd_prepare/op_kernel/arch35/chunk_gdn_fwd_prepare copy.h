...
public:
    ...
    __aicore__ inline void InitOnChip()
    {
        ...
        // Also gate-raw scratch after q' is in GM, and S5 Cast of A to bf16.
    ...
    __aicore__ inline void Stage0_GenerateResidentAux()
    {
        ...
        if (subBlock == 0) {
            constexpr int32_t nI = static_cast<int32_t>(kChunk64 * kChunk64);
            Duplicate(ubS0Zero, 0.0f, nI);
            SetFlag<HardEvent::V_S>(0);
            WaitFlag<HardEvent::V_S>(0);
            for (uint32_t i = 0; i < kChunk64; ++i) {
                ubS0Zero.SetValue(i * kChunk64 + i, 1.0f);
            }
    ...
    __aicore__ inline void Stage1_OneTask(const ChunkRange &chunk, int64_t hv, int64_t taskIdx)
    {
        ...
        GdnL2Norm::L2NormFwdK128VF<InDtype>(ubK[db], ubKHat, ubKRstd,
                                            static_cast<uint32_t>(nValid), kGdnL2NormEps);
        SetFlag<HardEvent::V_MTE3>(2);
        WaitFlag<HardEvent::V_MTE3>(2);
        for (uint16_t frac = 0; frac < 8; ++frac) {
            DataCopy(l1K[static_cast<int32_t>(frac) * 16 * 64],
                     ubKHat[static_cast<int32_t>(frac) * 16],
                     DataCopyParams(64, 1, 7, 0));
        }
        ...
        SetFlag<HardEvent::MTE2_V>(0);
        WaitFlag<HardEvent::MTE2_V>(0);
        GdnL2Norm::L2NormFwdK128VF<InDtype>(ubQ[db], ubQHat, ubQRstd,
                                            static_cast<uint32_t>(nValid), kGdnL2NormEps);
        SetFlag<HardEvent::V_MTE3>(2);
        WaitFlag<HardEvent::V_MTE3>(2);
        ...
        LocalTensor<float> ubGfp = ubG[db];
        LocalTensor<float> ubBfp = ubBeta[db];
        Duplicate(ubGfp, 0.0f, nPad);
        Duplicate(ubBfp, 0.0f, nPad);
        if constexpr (IsSameType<GateDtype, float>::value) {
            SetFlag<HardEvent::V_MTE2>(1);
            WaitFlag<HardEvent::V_MTE2>(1);
            DataCopy(ubGfp, gmG[offG], nValid);
            DataCopy(ubBfp, gmBeta[offG], nValid);
            SetFlag<HardEvent::MTE2_V>(1);
            WaitFlag<HardEvent::MTE2_V>(1);
        } else {
            LocalTensor<GateDtype> ubGRaw = ubQHat.template ReinterpretCast<GateDtype>();
            LocalTensor<GateDtype> ubBRaw = ubGRaw[nPad];
            SetFlag<HardEvent::V_MTE2>(1);
            WaitFlag<HardEvent::V_MTE2>(1);
        ...
        if (useGateInKernel != 0) {
            ...
            GateSoftplusVF<float>(ubGfp, ubGateTmp, aLog, dt, static_cast<uint32_t>(nValid));
            DataCopy(ubGfp, ubGateTmp, nPad);
        }
        ...
        if (useBetaSigmoid != 0) {
            ...
            BetaSigmoidVF<float>(ubBfp, ubGateTmp, bscale, static_cast<uint32_t>(nValid));
            DataCopy(ubBfp, ubGateTmp, nPad);
        }
        DataCopy(ubGPrime[db], ubGfp, nPad);
        DataCopy(ubBetaEff[db], ubBfp, nPad);
